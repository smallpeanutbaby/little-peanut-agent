...
- 设置页新增渠道机器人面板，统一管理 `QQ`、`飞书`、`钉钉` 机器人配置
- 支持保存 `AppId`、密钥、白名单来源、默认项目、默认模式和沙箱开关
- 主进程已提供列表、保存、状态查询、连通性测试和重启入口
- 当前实际打通主线以 `QQ 私聊机器人` 为先，支持会话状态保存、默认项目路由和沙箱/正式环境切换
...
在「设置 → 渠道机器人」启用 QQ 后，私聊机器人可用以下指令（推荐带前缀，避免数字歧义）：

| 指令 | 作用 |
|------|------|
| `我的项目` | 选择工作区项目 |
| `我的模型` / `模型 moonshot kimi-k2.6` | 指定 QQ 使用的服务商与模型（全模式生效） |
| `模式 agent` / `模式 chat` / `模式 3` | 切换 chat / agent / plan 等模式 |
| `agent` / `chat` / `plan` | 同上（整句匹配） |
| `开始` / `状态` | 查看就绪卡片与当前配置 |
| 直接发文字 | 在已选项目下提问（默认 chat；agent 下短句自动走 chat） |

纯数字仅在刚展示列表后 **10 分钟内**有效；已选项目后发 `3` 会切换到 plan，而不会当成「第 3 个项目」。
