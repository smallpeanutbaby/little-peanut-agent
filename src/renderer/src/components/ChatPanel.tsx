...
import type { ChatAttachment, ChatMessage, Conversation, ReviewScope, ThinkBudget } from "@shared/types";
import { AgentMessageList } from "./AgentMessageList";
import { TodoPanel } from "./TodoPanel";
import { CostBadge } from "./CostBadge";
import { useStickToBottomScroll } from "../hooks/useStickToBottomScroll";
import { formatMessageForDisplay, formatUserFacingError, sanitizeDisplayText } from "../utils/displayText";
...
interface ChatPanelProps {
  ...
  activeModeId: string;
  /** Review mode — configured scope for the active conversation. */
  reviewScope?: ReviewScope;
  /** Open the review setup wizard (reconfigure scope). */
  onConfigureReview?: () => void;
  /**
   * Bumped by the owner when the main process persists messages mid-run
   * (e.g. review diff failure) so we reload `listMessages` without waiting
   * for stream terminal.
   */
...
function ReviewScopeBanner({
  ...
  return (
    <div className="mx-auto flex w-full max-w-[860px] items-center justify-between gap-3 px-6 py-1.5">
      <div className="flex min-w-0 flex-1 items-center gap-2 rounded-full border border-violet-400/30 bg-[#1a1528] px-3 py-1.5 text-[12px] text-[var(--lp-text)]">
        <span className="font-medium text-violet-200">{t("modes.review.name")}</span>
        <span className="text-[var(--lp-muted)]">·</span>
        <span className="truncate text-[var(--lp-soft-text)]">
          {scope?.label ?? t("review.banner.notConfigured")}
        </span>
      </div>
      {onConfigure ? (
        <button
          type="button"
          onClick={onConfigure}
          className="shrink-0 rounded-full border border-[var(--lp-border)] px-3 py-1 text-[12px] text-[var(--lp-text)]/80 hover:bg-white/[0.04]"
        >
          {scope ? t("review.banner.reconfigure") : t("review.banner.configure")}
        </button>
      ) : null}
    </div>
  );
...
function ContextRing({ budget, compaction, trimmed }: {
  ...
  const displayUsed = budget ? Math.min(budget.usedTokens, denom) : 0;
  const pct = budget && denom > 0
    ? Math.min(100, Math.round((displayUsed / denom) * 100))
    : 0;
  ...
  const label = budget
    ? `${formatTokensShort(displayUsed)} / ${formatTokensShort(denom)}`
    : trimmed
      ? `已丢弃 ${trimmed.dropped} 条`
      : "";

  const tooltip = [
    budget ? `上下文窗口: ${displayUsed.toLocaleString()} / ${denom.toLocaleString()} tokens (${pct}%)` : "",
    budget?.compacted ? "已自动压缩（UI 仍显示完整历史）" : "",
...
export function ChatPanel({
  ...
  /** Send / validation errors only — stream errors are derived from `liveStream`. */
  ...
  const addFiles = useCallback(
    async (files: File[]) => {
      ...
      if (!attachImagesAllowed) {
        setSendError(t("chatPanel.attachVisionUnsupported"));
        return;
      }
      ...
      if (reasons.length > 0) setSendError(reasons.join("；"));
    },
  ...
  useEffect(() => {
    if (streaming) setSendError(null);
  }, [streaming]);
  ...
  const {
    ...
    showJumpButton,
    scrollToBottom
  } = useStickToBottomScroll(conversation?.id, [
    messages,
    liveStream?.text,
    liveStream?.reasoning,
    streaming
  ]);
  ...
  }, [conversation?.id, liveStream?.status, messagesRefreshSignal]);

  // Auto-scroll is handled by useStickToBottomScroll (only when near bottom).
  ...
  const send = useCallback(async () => {
    ...
    const isReview = activeModeId === "review";
    if (streaming) return;
    if (!isReview && !hasText && !hasAttachments) return;
    if (isReview && !reviewScope) {
      onConfigureReview?.();
      setSendError(t("review.setup.required"));
      return;
    }
    // Review already injects diff on「开始审查」— empty Send must not spawn another
    // full run (each failure used to leave a permanent ⚠️ row in the thread).
    if (isReview && !hasText && !hasAttachments) {
      setSendError(t("review.setup.followUpHint"));
      return;
    }
    setSendError(null);
    ...
    const result = await onStartStream({
      ...
      reviewScope: activeModeId === "review" ? reviewScope : undefined
    });
    if (!result.ok) {
      ...
      setSendError(sanitizeDisplayText(map[result.reason] || labels.deliveryError));
    ...
    onConversationsChanged?.();
  }, [input, attachments, streaming, conversation, defaultProviderId, defaultModelId, defaultThinkBudget, defaultThinkEnabled, activeModeId, reviewScope, onConfigureReview, t, onEnsureConversation, onStartStream, onConversationsChanged, labels]);
  ...
  const streamComposerError = useMemo(() => {
    if (liveStream?.status !== "error" || !liveStream.errorMessage) return null;
    const tail = groupedMessages[groupedMessages.length - 1];
    if (tail?.role === "assistant" && (tail.content ?? "").trim().startsWith("⚠️")) {
      return null;
    }
    return formatUserFacingError(liveStream.errorMessage, undefined, locale);
  }, [liveStream?.status, liveStream?.errorMessage, groupedMessages, locale]);

  const composerError = sendError ?? streamComposerError;
  ...
  const inProjectReview =
    activeModeId === "review" && !!conversation?.projectId;
  const suppressEmptyForStream = streaming || (liveStream?.text?.length ?? 0) > 0;
  /** Scope configured but DB rows not loaded yet — avoid generic chat hero. */
  const suppressEmptyForReviewScope = inProjectReview && !!reviewScope;
  const isEmpty =
    renderedMessages.length === 0 &&
    !suppressEmptyForStream &&
    !suppressEmptyForReviewScope;
  if (suppressEmptyForStream && !conversation) {
    console.warn("[lp/chat-panel] streaming with no conversation prop — upstream desync");
  }

  const showReviewPreparing =
    inProjectReview &&
    !!reviewScope &&
  ...
  const failedAssistantId = useMemo(() => {
    ...
    const isErrorMarker = (tail.content ?? "").trim().startsWith("⚠️");
  ...
  const handleRetry = useCallback(async () => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        setSendError(sanitizeDisplayText(map[result.reason] || labels.retryFailed));
      }
    } finally {
      setRetrying(false);
    }
  ...
  return (
    <div className="flex h-full w-full flex-col">
      <div className="relative min-h-0 flex-1">
        ...
        <div ref={listRef} className="h-full overflow-y-auto px-6 pt-6 pb-4">
        {isEmpty ? (
          ...
          (activeModeId === "agent" || activeModeId === "plan" || activeModeId === "pipeline" || activeModeId === "review") ? (
          ...
          <>
            ...
            {(activeModeId === "agent" || activeModeId === "plan") ? (
              <TodoPanel
            ...
            {showReviewPreparing ? (
              <p className="mx-auto mb-4 w-full max-w-[860px] px-1 text-center text-[13px] text-[var(--lp-soft-text)]">
                {t("review.preparing")}
              </p>
            ) : null}
            <AgentMessageList
              ...
              liveAssistantOverlay={
                liveStream?.status === "streaming"
                  ? { text: liveStream.text ?? "", reasoning: liveStream.reasoning ?? "" }
                  : undefined
              }
        ...
        {showJumpButton ? (
          <button
            ...
            className="absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1.5 rounded-full border border-[var(--lp-border)] bg-[var(--lp-panel)]/95 px-3.5 py-1.5 text-[12px] text-[var(--lp-text)]/85 shadow-[0_4px_16px_rgba(0,0,0,0.28)] backdrop-blur transition hover:bg-[var(--lp-panel-2)]"
          >
            <span aria-hidden>↓</span>
            <span>{t("chatPanel.scrollToBottom", { defaultValue: "回到底部" })}</span>
          </button>
        ) : null}
      </div>
      ...
      {activeModeId === "review" ? (
        <ReviewScopeBanner scope={reviewScope} onConfigure={onConfigureReview} />
      ) : null}
...
function MessageBubble({ message, youLabel, assistantLabel, reasoningLabel, isStreaming, isFailed, retrying, retryLabel, onRetry }: {
  ...
  const locale = i18n.language === "en" ? "en" : "zh";
  const content = formatMessageForDisplay(message.content ?? "", locale);
  const isErrorMarker = !isUser && content.trim().startsWith("⚠️");
  return (
